...
class CoroutineFunctionTransformer(private val function: JsFunction, name: String?) {
    ...
    private fun isTailCall(): Boolean {
        ...
        if (suspendCalls.size == 0) return true
        if (suspendCalls.size > 1) return false

        val suspendCall = suspendCalls.single()

        if (body.statements.size < 2) return false
        val lastStatement = body.statements.last() as? JsReturn ?: return false
        if (!lastStatement.expression.isStateMachineResult()) return false

        val statementBeforeLast = body.statements[body.statements.lastIndex - 1] as? JsExpressionStatement ?: return false
        val suspendExpression = statementBeforeLast.expression
        if (suspendExpression == suspendCall) return true

        decomposeAssignment(suspendExpression)?.let { (lhs, rhs) ->
            if (rhs == suspendCall && lhs.isStateMachineResult()) return true
        }
