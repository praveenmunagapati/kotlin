 ...
 */
fun stripMetadata(project: Project, classNamePattern: String, inFile: File, outFile: File) {
    ...
    project.logger.info("Stripping @kotlin.Metadata annotations from all classes in $inFile")
    project.logger.info("Class name pattern: $classNamePattern")
    project.logger.info("Input file size: ${inFile.length()} bytes")
    project.logger.info("Output written to $outFile")
    project.logger.info("Output file size: ${outFile.length()} bytes")
}
